function [analyse]=calculdiffusion(current,tps_global_zones,a,Te,nb_fit,seuil,waitbarhandle)
%function [analyse]=calculdiffusion(current,tps_global_zones,a,Te,nb_fit,seuil,waitbarhandle)
...
for ind=1:length(current.spot)
    ...
    if isfield(current.spot,'localisation')
       coord=current.spot(ind).localisation(1).coord;
    else
        coord=[];
        for k=1:current.spot(ind).nb_segments
            coord=[coord; current.spot(ind).segment(k).coordinates];
        end
    end
    %disp(coord)
    if size(coord,2)>3
   ...
   %disp(size(coord))
    ...
    if isfield(current.spot,'localisation')
       analyse.nb_frames=length(current.spot(1).localisation(1).coord(:,1));
    else
       analyse.nb_frames=length(coord);
    end
